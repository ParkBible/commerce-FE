...
// 편집 아이콘
export const EditIcon = ({ title = "편집 아이콘", ...props }: Omit<IconProps, "direction" | "size" | "strokeWidth">) => {
    const uniqueId = useId();
    const titleId = `edit-icon-${uniqueId}`;

    return (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" role="img" aria-labelledby={titleId} {...props}>
            <title id={titleId}>{title}</title>
