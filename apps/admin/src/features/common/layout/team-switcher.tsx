        ...
        <div className="relative w-full px-2">
            ...
            >
                <div className="flex items-center gap-2">
                    <div className="grid h-6 w-6 place-items-center rounded bg-primary text-white">{selectedTeam.name.charAt(0)}</div>
                    {open && (
                        <div>
                ...
                {open && <ChevronDown className={`h-4 w-4 text-gray-500 transition-transform ${isOpen ? "rotate-180" : ""}`} />}
            </button>
                        ...
                        {teams.map(team => (
                            <button
                                ...
                                className={`flex w-full items-center gap-2 px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-zinc-800 ${
                                    selectedTeam.value === team.value ? "bg-gray-100 dark:bg-zinc-800" : ""
                                }`}
                                onClick={() => selectTeam(team)}
                            >
                                <div className="grid h-6 w-6 place-items-center rounded bg-primary text-white">{team.name.charAt(0)}</div>
